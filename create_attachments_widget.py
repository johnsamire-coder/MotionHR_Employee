# -*- coding: utf-8 -*-
"""
سكريبت ينشئ AttachmentsWidget.dart
"""
import os

CONTENT = '''// lib/widgets/attachments_widget.dart
// Attachments Widget - Reusable widget for file attachments (Phase 4)
...
class _AttachmentsWidgetState extends State<AttachmentsWidget> {
  ...
  Future<void> _pickAndUpload(String source) async {
    ...
    if (widget.objectId == null) {
      // Object not yet created (e.g., new request)
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
...
with open("lib/widgets/attachments_widget.dart", "w", encoding="utf-8") as f:
    f.write(CONTENT)

print("OK - attachments_widget.dart created:", len(CONTENT), "bytes")