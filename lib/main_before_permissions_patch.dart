...
Future<void> saveFCMTokenToServer() async {
  try {
    ...
    if (token == null || token.isEmpty) {
      print('⚠️ No auth token, skipping FCM registration');
      return;
    }
    ...
    if (res.statusCode == 200) {
      ...
      print('⚠️ Failed to save FCM token: ${res.statusCode}');
    }
  } catch (e) {
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _checkAuth() async {
    ...
    if (token != null && token.isNotEmpty) {
      // ⭐ لو المستخدم عامل Login، احفظ FCM Token
    ...
    try {
      ...
      if (res.statusCode == 200 && data['success'] == true) {
        ...
        if (data['employee'] is Map) {
          fullName =
              fullName.isEmpty ? (data['employee']['name'] ?? '') : fullName;
          companyName =
              companyName.isEmpty
          ...
          firstName =
              firstName.isEmpty
                  ...
                  : firstName;
          gender =
              gender == 'male' && data['employee']['gender'] != null
      ...
      builder:
          (_) => Directionality(
            ...
            child: AlertDialog(
              ...
              content: const Text(
                'من فضلك تواصل مع مسؤول الموارد البشرية لإعادة تعيين كلمة المرور الخاصة بك.',
              ),
              actions: [
                TextButton(
...
class _EmployeeHomeScreenState extends State<EmployeeHomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final displayName =
        _firstName.isNotEmpty ? _firstName : (_fullName.isEmpty ? "بيك" : _fullName);
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'شيفت: $shiftName ($shiftStart - $shiftEnd)',
          ...
          if (checkedIn && !checkedOut)
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(16),
                border: Border.all(
                  color: canCheckOut ? Colors.green[200]! : Colors.blue[200]!,
                ),
              ),
              child: Column(
                children: [
                  ...
                  Text(
                    '${(_progressValue() * 100).toInt()}% من الشيفت',
              ...
              children: [
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Column(
                        ...
                        children: [
                          Icon(
                            checkedIn ? Icons.check_circle : Icons.login,
                            size: 40,
                          ),
                          const SizedBox(height: 6),
                          Text(
                ...
                Expanded(
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed:
                          (_loading || !checkedIn || (!canCheckOut && !hasEarlyLeave))
                              ? null
...
class _LeavesScreenState extends State<LeavesScreen> {
  ...
  int _orderKey(Map t) {
    ...
    if (category == 'sick' || name.contains('مرضية') || name.contains('مرضي')) {
      return 3;
    }
    return 4;
  }
        ...
        ..._types.map((t) {
          ...
          return Card(
            child: ListTile(
              ...
              title: Text(t['name'] ?? ''),
              subtitle: Text('الرصيد المتبقي: $balance يوم'),
            ),
          );
        }),
        const SizedBox(height: 20),
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(data['message'] ?? 'تم')));
...
class _RequestsScreenState extends State<RequestsScreen> {
  ...
  bool get _isOther => _selectedValue == 'other';
  bool get _isLoan => _types.any((t) => t['id'].toString() == _selectedValue && ((t['name']?.toString().contains('سلفة') ?? false) || (t['name']?.toString().contains('سلف') ?? false)));
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final body = <String, dynamic>{
        ...
        if (_isLoan && _amountCtrl.text.isNotEmpty) 'amount': _amountCtrl.text,
        'description':
            _isOther
                ? 'نوع آخر: ${_otherCtrl.text}\n${_descCtrl.text}'
                : _descCtrl.text,
      ...
      child: Column(
        children: [
          const TabBar(
            ...
            tabs: [Tab(text: 'طلباتي'), Tab(text: 'إجازاتي')],
          ),
          Expanded(
            child: TabBarView(
              children: [
...
class _MyListState extends State<_MyList> {
  ...
  Color _statusColor(String s) {
    ...
    if (s.contains('رفض') || s.toLowerCase().contains('reject')) {
      return Colors.red;
    }
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _markAllRead() async {
    ...
    try {
      await http.post(
        Uri.parse('$kBaseUrl/attendance/api/mobile/notifications/mark-read/'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Token $token',
        },
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 6),
                              Text(
                                'خط العرض: ${_currentLat!.toStringAsFixed(6)}',
                              ),
                              Text(
                                'خط الطول: ${_currentLng!.toStringAsFixed(6)}',
                              ),
                            ],
                          ),
                        ),
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (r1.statusCode == 200) {
        ...
        final pendingRequests = ((d['pending_requests'] as List?) ?? []).length;
        final pendingLeaves = ((d['pending_leaves'] as List?) ?? []).length;
        final pendingLegacy = ((d['pending'] as List?) ?? []).length;
        final totalPending = d['total_pending'];
        if (totalPending is num) {
          _pending = totalPending.toInt();
        } else {
          _pending = pendingRequests + pendingLeaves + pendingLegacy;
        }
      }
      ...
      if (r2.statusCode == 200) {
        ...
        final total = d['total'];
        if (total is num) {
          _present = total.toInt();
        } else {
          _present = items.length;
        }
      }
      ...
      child: ListView(
        ...
        children: [
          ...
          _card(
            ...
            () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => const ManagerAttendanceScreen()),
            ),
          ),
          const SizedBox(height: 12),
          _card(
          ...
          _card(
            ...
            () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => const ManagerGeofenceScreen()),
            ),
          ),
        ],
      ),
...
class _ManagerPendingScreenState extends State<ManagerPendingScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        final requests = List<dynamic>.from(data['pending_requests'] ?? []);
        final leaves = List<dynamic>.from(data['pending_leaves'] ?? []);
        final legacy = List<dynamic>.from(data['pending'] ?? []);
        final merged = [...requests, ...leaves, ...legacy];
  ...
  Future<void> _action(dynamic item, String action) async {
    ...
    try {
      final res = await http.post(
        Uri.parse('$kBaseUrl/attendance/api/mobile/manager/action/'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Token $token',
        },
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('حدث: $e')));
      }
    }
  }
...
class _ManagerAttendanceScreenState extends State<ManagerAttendanceScreen> {
  ...
  Widget build(BuildContext context) {
    if (_loading) return const Center(child: CircularProgressIndicator());
    if (_items.isEmpty) return const Center(child: Text('لا توجد سجلات حضور اليوم'));
    return RefreshIndicator(
      onRefresh: _load,
      child: ListView.builder(
        itemCount: _items.length,
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: _showMap ? kManagerColor : Colors.grey,
                    foregroundColor: Colors.white,
                  ),
                ),
              ),
