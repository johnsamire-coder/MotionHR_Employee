...
class _ManagerMissionsScreenState extends State<ManagerMissionsScreen>
  ...
  List<dynamic> get _filtered {
    if (_search.trim().isEmpty) return _missions;
    final s = _search.toLowerCase().trim();
    return _missions.where((m) {
      final title = (m['title'] ?? '').toString().toLowerCase();
      final desc = (m['description'] ?? '').toString().toLowerCase();
      final client = (m['client_name'] ?? '').toString().toLowerCase();
      return title.contains(s) || desc.contains(s) || client.contains(s);
    }).toList();
  }
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ListView(
                  ...
                  children:
                      _getStatusLabels(context).entries.map((entry) {
  ...
  Widget _buildMissionCard(Map<String, dynamic> mission) {
    ...
    // ─── Feedback Badge ───
    final hasFeedback = (mission['feedback_count'] != null &&
            mission['feedback_count'] != 0) ||
        (mission['has_feedback'] == true);
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(
                    _priorityIcons[priority] ?? Icons.remove,
                    color: priorityColor,
                    size: 18,
                  ),
              ...
              if ((mission['location_name'] ?? '').toString().isNotEmpty)
                Row(
                  children: [
        ...
        return ListView(
          ...
          children: [
            GridView.count(
              ...
              children: [
                _statCard(
                  context.l10n.veryInterested,
                  '${summary['very_interested'] ?? 0}',
                  Icons.thumb_up,
                  Colors.green,
                ),
                _statCard(
  ...
  Widget _buildFeedbackCard(Map<String, dynamic> fb, bool isAr) {
    return Card(
      ...
      child: InkWell(
        ...
        onTap: () => _showFeedbackDetail(fb, isAr),
        child: Padding(
          padding: const EdgeInsets.all(12),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
        ...
        child: AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              mainAxisSize: MainAxisSize.min,
              children: [
                _detailRow(Icons.person, isAr ? 'الموظف' : 'Employee',
                    fb['employee_name'] ?? '-'),
                _detailRow(Icons.business, isAr ? 'العميل' : 'Client',
                    fb['client_name'] ?? '-'),
                _detailRow(Icons.phone,
                    isAr ? 'هاتف العميل' : 'Client Phone',
                    fb['client_phone'] ?? '-'),
                _detailRow(
                    Icons.star,
                ...
                _detailRow(
                    Icons.handshake,
                    isAr ? 'تم توقيع عقد' : 'Contract Signed',
                    (fb['contract_signed'] == true)
                        ? (isAr ? 'نعم' : 'Yes')
                        : (isAr ? 'لا' : 'No')),
                _detailRow(
                    Icons.follow_the_signs,
                    isAr ? 'يحتاج متابعة' : 'Needs Follow-up',
                    (fb['needs_followup'] == true)
                        ? (isAr ? 'نعم' : 'Yes')
                        : (isAr ? 'لا' : 'No')),
                _detailRow(
