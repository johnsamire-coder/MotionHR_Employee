...
class _LateReportScreenState extends State<LateReportScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      _data = await _service.getLateReport(
        year: _selectedYear,
    ...
    try {
      ...
      final rows = employees.map<List<String>>((e) {
        ...
        return [
          ...
          '${item['total_late_days'] ?? 0}',
          '${item['total_late_minutes'] ?? 0}',
          '${item['total_late_hours'] ?? 0}',
        ];
      }).toList();
      await ReportPdfService.printReport(
        title: _isAr ? 'تقرير التأخير' : 'Late Report',
  ...
  Future<void> _exportExcel() async {
    ...
    try {
      ...
      await ReportExcelService.exportLateReport(
        employees: employees,
        year: _selectedYear,
        month: _selectedMonth,
        isAr: _isAr,
  ...
  Widget build(BuildContext context) {
    ...
    final title = _isAr ? 'تقرير التأخير' : 'Late Report';
                ...
                children: [
                  ...
                  SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        _chip(
                          _isAr ? 'موظفين متأخرين' : 'Late Employees',
  ...
  Widget _buildCard(Map<String, dynamic> emp) {
    ...
    final lateDays = emp['total_late_days'] ?? 0;
    final lateMin = emp['total_late_minutes'] ?? 0;
    final lateHrs = emp['total_late_hours'] ?? 0;
    final details = (emp['details'] as List?) ?? [];
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: Text(
          _isAr
              ? 'تأخير: $lateDays يوم | $lateMin دقيقة'
              : 'Late: $lateDays days | $lateMin min',
        ...
        children: [
          ...
          Padding(
            ...
            child: Column(
              children: [
                _detailRow(
                  _isAr ? 'أيام التأخير' : 'Late Days',
                  '$lateDays',
                ),
                _detailRow(
                  _isAr ? 'إجمالي الدقائق' : 'Total Minutes',
                  '$lateMin',
                ),
                _detailRow(
          ...
          if (details.isNotEmpty) ...[
            ...
            ...details.map<Widget>((d) {
              ...
              return ListTile(
                ...
                subtitle: Text(
                  '${_isAr ? 'وقت الحضور' : 'Check-in'}: ${day['time'] ?? '-'}',
                  style: TextStyle(
                      fontSize: 11, color: Colors.grey[600]),
                ),
                trailing: Text(
                  '${day['minutes_late'] ?? 0} ${_isAr ? 'د' : 'm'}',
                  style: const TextStyle(
                    color: _color,
                    fontWeight: FontWeight.bold,
                    fontSize: 13,
