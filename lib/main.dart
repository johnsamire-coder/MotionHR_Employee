        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Expanded(
                        child: Text('يجب تغيير كلمة المرور قبل استخدام التطبيق',
              ...
              const SizedBox(height: 20),
              _pf(_currentCtrl, 'كلمة المرور الحالية', _obscure1,
                  () => setState(() => _obscure1 = !_obscure1)),
              const SizedBox(height: 16),
              _pf(_newCtrl, 'كلمة المرور الجديدة', _obscure2,
                  () => setState(() => _obscure2 = !_obscure2)),
              const SizedBox(height: 16),
              _pf(_confirmCtrl, 'تأكيد كلمة المرور', _obscure3,
...
class _ManagerShellState extends State<ManagerShell> {
  ...
  Future<void> _logout() async {
    final prefs = await SharedPreferences.getInstance(); await prefs.clear(); unreadNotificationsCount.value = 0;
...
class _ManagerDashboardState extends State<ManagerDashboard> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (r1.statusCode == 200) { final d = jsonDecode(r1.body);
        final pr = ((d['pending_requests'] as List?) ?? []).length; final pl = ((d['pending_leaves'] as List?) ?? []).length;
        final pg = ((d['pending'] as List?) ?? []).length; final tp = d['total_pending']; _pending = tp is num ? tp.toInt() : pr + pl + pg; }
      final r2 = await http.get(Uri.parse('$kBaseUrl/attendance/api/mobile/manager/attendance/'), headers: {'Authorization': 'Token $token'});
      if (r2.statusCode == 200) { final d = jsonDecode(r2.body);
        final items = ((d['items'] as List?) ?? (d['attendance'] as List?) ?? []); final total = d['total']; _present = total is num ? total.toInt() : items.length; }
      ...
      if (r3.statusCode == 200) { final d = jsonDecode(r3.body); _fieldWorkers = ((d['locations'] as List?) ?? []).length; }
    } catch (_) {}
    ...
    return Card(elevation: 3, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(16),
        child: Padding(padding: const EdgeInsets.all(20), child: Row(children: [
          Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: color.withOpacity(0.15), borderRadius: BorderRadius.circular(12)),
            child: Icon(icon, size: 40, color: color)),
          const SizedBox(width: 16),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(title, style: const TextStyle(fontSize: 16, color: Colors.grey)), const SizedBox(height: 4),
...
class _ManagerLiveLocationsScreenState extends State<ManagerLiveLocationsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_items.isEmpty) return Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Icon(Icons.location_off, size: 80, color: Colors.grey[400]), const SizedBox(height: 16),
      const Text('لا توجد مواقع لحظية', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 8),
      Text('لعرض المواقع، تأكد من:\n• يوجد موظفين ميدانيين\n• تفعيل التتبع لهم\n• تشغيل تطبيقاتهم', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey[600])),
      const SizedBox(height: 16), ElevatedButton.icon(onPressed: _load, icon: const Icon(Icons.refresh), label: const Text('تحديث'))])));
