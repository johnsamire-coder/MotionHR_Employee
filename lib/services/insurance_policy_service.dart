...
class InsurancePolicyService {
  ...
  static Future<List<dynamic>> listPolicies() async {
    final res = await http.get(
      Uri.parse('$_baseUrl$_basePath/insurance-policies/'),
  ...
  static Future<Map<String, dynamic>> getPolicy(int id) async {
    final res = await http.get(
      Uri.parse('$_baseUrl$_basePath/insurance-policies/$id/'),
      headers: await _headers(),
    );
    if (res.statusCode == 200) {
      final data = json.decode(utf8.decode(res.bodyBytes));
  ...
  /// ??? ??????? ????
  static Future<Map<String, dynamic>> getEmployeeInsurances(int employeeId) async {
    final res = await http.get(
      Uri.parse('$_baseUrl$_basePath/employees/$employeeId/insurances/'),
      headers: await _headers(),
    );
    if (res.statusCode == 200) {
      return json.decode(utf8.decode(res.bodyBytes));
    }
    throw Exception('Failed to load employee insurances');
  }
}
