# -*- coding: utf-8 -*-
"""
سكريبت ينشئ AttachmentService.dart
"""
import os

CONTENT = '''// lib/services/attachment_service.dart
// Attachment Service - Handles all file operations (Phase 4)
  ...
  static Future<UploadResult> upload({
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        return UploadResult(
          success: true,
          attachment: AttachmentModel.fromJson(data['attachment']),
        );
      } else {
        final data = jsonDecode(utf8.decode(response.bodyBytes));
        return UploadResult(
          success: false,
          error: data['error'] ?? 'خطأ في الرفع',
        );
      }
    } catch (e) {
...
with open("lib/services/attachment_service.dart", "w", encoding="utf-8") as f:
    f.write(CONTENT)

print("OK - attachment_service.dart created:", len(CONTENT), "bytes")